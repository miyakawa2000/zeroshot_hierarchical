...
class SAMVisualizationDemo(object):
    def __init__(self, cfg, granularity, sam_path, ovsegclip_path, instance_mode=ColorMode.IMAGE, parallel=False):
        ...
        self.predictor = SamAutomaticMaskGenerator(
            ...
            points_per_batch=16, # The number of points runs simultaneously by the model
            pred_iou_thresh=0.86, # A filtering thresh of model's mask quality
            stability_score_thresh=0.92, # A filtering thresh of the stability of the masks
            crop_n_layers=1, # If > 0, mask prediction will run again on crops of the image.
            crop_n_points_downscale_factor=2, # The number of points_per_side sampled in layer n is scaled down by CnPDF**2 
    ...
    def run_on_image(self, image, class_names: list):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            class_preds = (100.0 * image_features @ text_features.T).softmax(dim=-1) # calc similarity # shape N_masks x N_classes
        
        # calc threshold for each class
        max_scores, select_mask = torch.max(class_preds, dim=0) # max score for each class # and idxs of mask whose mask score is max for each class
        if len(class_names) == 2 and class_names[-1] == 'others':
            select_mask = select_mask[:-1]
        
        # make select_mask (the list of idxs of masks whose mask score is larger than threshold)
        if self.granularity < 1:
            thr_scores = max_scores * self.granularity
            select_mask = []
            if len(class_names) == 2 and class_names[-1] == 'others':
                thr_scores = thr_scores[:-1]
            for i, thr in enumerate(thr_scores): # for each class
                cls_pred = class_preds[:,i]
                locs = torch.where(cls_pred > thr)
                select_mask.extend(locs[0].tolist())
        
        # make insseg output
        inssegs = [[] for _ in range(len(class_names))]
        ...
        pred_mask = r.argmax(dim=0).to('cpu')
        pred_mask[blank_area] = 255
        pred_mask = np.array(pred_mask, dtype=np.int) # (H, W)

        vis_output = visualizer.draw_sem_seg(
            pred_mask
        )

        return None, vis_output, inssegs